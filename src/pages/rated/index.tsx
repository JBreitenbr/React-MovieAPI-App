...
const {data:ratedTvShows,isLoading:isLoadingTvShows} = useQuery({queryKey:["ratedTvShows"],queryFn:fetchRatedTvShows})
  ...
  return (
    <Container style={{marginTop:"80px"}}><Menu pointing secondary>
        ...
        <Segment>{isLoadingMovies||isLoadingTvShows?(<div>Loading...</div>):(<div style={{marginTop:20}}>
            {displayType==DisplayType.Movies?(
        <ColumnDisplay data={ratedMovies.results} displayType={DisplayType.Movies}/>):
            (<ColumnDisplay data={ratedTvShows.results} displayType={DisplayType.TvShows}/>)}</div>)}</Segment>
    </Container>)
}
